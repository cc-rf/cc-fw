...
def parse(data):
    ...
    while len(data):
        ...
        # new method:
        # tag, dev_id, pkt_id, p_counter = unpack('<3s2pB3p', data)

        dev_id = ':'.join("{:02x}".format(ord(c)) for c in data[3:5])
        pkt_id = ord(data[5])
        p_counter = sum(ord(c) << (8 * i) for i, c in enumerate(data[6:9]))

        data_len = PKT_DATA_LEN_MAP.get(pkt_id, 0)
        # print "data_len=%i len(data[13:])=%i" % (data_len, len(data[13:]))

        if data_len > len(data[9:]):
            break

        p_data = " ".join("{:02x}".format(ord(c)) for c in data[9:9 + data_len + 1])
