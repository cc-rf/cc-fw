...
bool itm_port_available(const uint8_t port)
{
    return (ITM->TER & (1UL << port)) != 0UL;
}

void itm_putch(const uint8_t port, const u8 ch)
{
    while (ITM->PORT[port].u32 == 0UL) __NOP();
    ITM->PORT[port].u8 = ch;
}

void itm_puts(const uint8_t port, const char *str)
{
    //if (!itm_available() || !itm_port_available(port)) return;
    char ch;

    while ((ch = *str++)) {
        itm_putch(port, (u8) ch);
    }
}

void itm_write(const uint8_t port, const uint8_t *buf, const size_t len)
{
    //if (!itm_available() || !itm_port_available(port)) return;
...
void itm_printf(const uint8_t port, const char *format, ...)
{
    va_list va;
    u8 *output;
    int result;
...
void itm_write16(const uint8_t port, const uint16_t *buf, const size_t len)
{
    //if (!itm_available() || !itm_port_available(port)) return;
...
void itm_write32(const uint8_t port, const uint32_t *buf, const size_t len)
{
    //if (!itm_available() || !itm_port_available(port)) return;
